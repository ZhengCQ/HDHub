    ...
    <el-aside width="250px" style="background-color: rgb(238, 241, 246); padding-top: 50px">
      <el-menu :default-openeds="['1', '2', '3']" :default-active="this.$route.path" router>
        ...
        <el-submenu index="3">
          ...
          <el-menu-item @click="showTabs(1);activeName='second'">Scatter of HDL VS LDSC</el-menu-item>
          <el-menu-item @click="showTabs(2);activeName='third'">NetGraph</el-menu-item>
        </el-submenu>
      </el-menu>
    </el-aside>
        ...
        <el-tabs v-model="activeName" type="card" ref="tabs">
          <el-tab-pane label="BarPlot of Genetic Correlation" name="first">
            <el-row style="background: #fff; padding: 0px 0px 0px 0px; width: 100%">
              ...
              <div v-if="barPlotData">
                <bar-plot :key="timer" :class-name="subName" :subName="traits[0]" :chart-data="barPlotData.data"
                  :col-data="barPlotData.col" :pval-data="barPlotData.pval" :error-data="barPlotData.se" />
              </div>
            </el-row>

            <rg-table @barPlotData="getBarData" :gwasIds="gwas_ids" :rgModel="rgmodel" :filterInfo="setParaInfo"
          ...
          <el-tab-pane label="Scatter of HDL VS LDSC" name="second">
            ...
            <div v-show="scatterData">
              <pair-table @ScatterPlotData="getScatterData" :filterInfo="setParaInfo" ref="pairtable">
              </pair-table>
            </div>
          </el-tab-pane>
          ...
          <el-tab-pane label="NetGraph" name="third">
            ...
            <el-row v-show="heatMapData.length > 0" style="background: #fff; padding: 0px 0px 0px 0px; width: 100%">
              <heat-map :className="traits[0]" :chart-data="heatMapData" :x-data="colHeatMapData"
                :y-data="colHeatMapData" />
            </el-row>

            <!--画图 开始-->
            <el-row v-show="netNodes.length > 0" style="background: #fff; padding: 0px 0px 0px 0px; width: 100%">
              <net-graph :className="traits[0]" :nodes="netNodes" :links="netLinks" :categories="netCategories" />
            </el-row>
            <!--画图 结束-->
            <net-table @PlotData="getNetData" :gwasIds="gwas_ids" :rgModel="rgmodel" :filterInfo="setParaInfo"
              ref="nettable">
            </net-table>
          </el-tab-pane>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      showTabs(idx) {
        this.$nextTick(() => {
          this.$refs.tabs.$children[0].$refs.tabs[idx].style.display = "inline-block";
        })
      },
      changeModel() {
